...
public:
  ...
  Tetromino(Type t, FPoint p = {0, 0}, Rotation r = Rotation::R0)
      : type(t), pos(p), rotation(r) {}

  void set_pos(FPoint p);
  void set_rotation(Rotation r);

  void shift(FPoint delta);
  auto try_shift(FPoint delta, const Matrix &matrix) -> bool;
  void hard_drop(const Matrix &matrix);

  auto srs_rotation(Rotation next_rotation, const Matrix &matrix) -> bool;

  [[nodiscard]] auto get_shape_at(FPoint offset) const -> Shape;
  [[nodiscard]] auto get_shifted_shape(FPoint delta) const -> Shape;
  ...
  [[nodiscard]] auto get_pos() const -> FPoint;
