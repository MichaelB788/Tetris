...
void UI::render(const GameState& gameState, Dimension2D windowSize) {
	...
	renderMatrix(gameState.getMatrix());
	renderTetromino(gameState.getQueue());
}
...
void UI::renderTetromino(const TetrominoQueue& queue) const {
	// Render stored
	...
	for (const auto& tet : queue.getNextQueue()) {
		offset.y += 600;
		TetrisRenderer::renderTetromino(queue.getHold(), renderer, offset);
	}
}
